...
```{r}
#| label: combine-three-basic-plots
...
knitr::include_graphics("../figures/fig-panel-per-year.png")
```

We collected data from `r total_number_of_ads_in_our_sample` ads for tenure-track jobs in archaeology posted during 2013-2023. Figure @show-basic-plots shows the count of ads for each year. Assistant Professor jobs are consistently the most common title and rank, and open rank or full professor the least frequent. The ratio of tenure-track to non-tenure track positions is generally well above one, with 
