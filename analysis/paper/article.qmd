...
Job ads frequently mention multiple topics in a single call for applications, more often than they they multiple methods or geographic locations. A Kruskal-Wallis test indicated significantly higher richness in topics compared to richness of geographic locations or methods in job ads (χ2 (df = `r unname(kw_test_richness$df)`, N = `r nrow(kw_test_richness_input)`)  = `r round(unname(kw_test_richness$statistic), 2)`, p = `r expSup(unname(kw_test_richness$p.value))`). @fig-show-cooc shows topic co-occurances in our sample. 
